...
const [BaseForm, baseFormApi] = useVbenForm({
  ...
  schema: [
    ...
    },
    {
      ...
      help: $t('page.menu.tips.redirect'),
      componentProps: {
        placeholder: $t('page.menu.tips.redirect'),
        allowClear: true,
      },
      dependencies: {
...
const [Drawer, drawerApi] = useVbenDrawer({
  ...
  async onConfirm() {
    ...
    // 获取表单数据 - pass directly to store, which handles cloning and transformations
    const values = await baseFormApi.getValues();

    console.log(getTitle.value, values);

    try {
      // Note: authority conversion is handled by menuStore.prepareMenuData
