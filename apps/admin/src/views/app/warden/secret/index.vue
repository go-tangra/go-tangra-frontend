...
import { type wardenservicev1_Secret } from '#/generated/api/admin/service/v1';
...
const gridOptions: VxeGridProps<wardenservicev1_Secret> = {
  height: 'auto',
  stripe: false,
  toolbarConfig: {
    custom: true,
...
function handleViewSecret(row: wardenservicev1_Secret) {
  openSecretDrawer(row, 'view');
}

function handleEditSecret(row: wardenservicev1_Secret) {
  openSecretDrawer(row, 'edit');
}

function handleCreateSecret() {
  openSecretDrawer({} as wardenservicev1_Secret, 'create');
}

async function handleDeleteSecret(row: wardenservicev1_Secret) {
  if (!row.id) return;
  try {
...
async function handleCopyPassword(row: wardenservicev1_Secret) {
  if (!row.id) return;
  try {
...
function handleViewVersions(row: wardenservicev1_Secret) {
  versionDrawerApi.setData({ secret: row });
  versionDrawerApi.open();
}

function handleViewPermissions(row: wardenservicev1_Secret) {
