...
const checkStrictly = ref(props.defaultCheckStrictly);
...
function getModalValue() {
  ...
  // When checkStrictly is true, ant-design tree expects { checked: [], halfChecked: [] }
  if (checkStrictly.value && Array.isArray(value)) {
...
const bindProps = computed(() => {
  return {
    ...
    [`onUpdate:${props.modelPropName}`]: (val: any) => {
      // When checkStrictly is true, tree returns { checked: [], halfChecked: [] }
      // We need to extract just the checked array for the model
      if (checkStrictly.value && val && typeof val === 'object' && 'checked' in val) {
        modelValue.value = val.checked;
      } else {
